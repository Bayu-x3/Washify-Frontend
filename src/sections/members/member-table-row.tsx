...
export function MemberTableRow({ row, selected, onSelectRow, onDeleteUser }: MemberTableRowProps) {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        setToastMessage(`Member "${row.nama}" has been deleted successfully.`);
        setToastSeverity('success');
        onDeleteUser(row.id);
      } else {
        const result = await response.json();
        setToastMessage(result.message || 'Failed to delete Member.');
        setToastSeverity('error');
      }
    } catch (error) {
      console.error('Error deleting Member:', error);
      setToastMessage('An error occurred while deleting the Member.');
      setToastSeverity('error');
    } finally {
      setToastOpen(true);
      setOpenDeleteDialog(false);
  ...
  return (
    <>
      ...
      <Dialog open={openDeleteDialog} onClose={handleDeleteDialogClose}>
        ...
        <DialogContent>
          Are you sure you want to delete Member {row.nama}?
        </DialogContent>
        <DialogActions>
